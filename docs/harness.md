...
* `squads_test.go` — M4.4c-1's script, the sixteenth: the player commands
  squads. One squad ships; the provider's flat face, its `squads[]` array and
  the sheet agree; a select-click selects and is consumed; a harness-placed
...
| 2 | 23 Sep 2026, M5.3, TestTownWalkDeterministic in the full suite | `systems` (`assets.strings`) | **A frame count in the string census.** The census counted how often each key was asked for, and the HUD asks for its four tooltips (`panelexp`, `panelhealth`, `panelmana`, `panelstamina`) every frame it draws -- so the count measured frames drawn while loading, which is the wall clock's business: 5 in one launch, 4 in the other. Found by diffing every system's state across two launches; nothing else differed. It had passed the suite before by luck. | **CLOSED the same day**: the census keeps each key once (found, text) and no count; `TestAskingAgainLeavesTheCensusAlone` (red on the counting census) guards it. Residual, noted: WHICH keys are on record still follows what was drawn -- a key asked only on a frame one launch draws and the other does not would diverge the same way. None does today (the per-system diff across two launches, run three times, is empty). |
