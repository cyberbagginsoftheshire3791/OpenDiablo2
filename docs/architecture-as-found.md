...
10. **There is no CI** *(resolved at M2.4, see §9)* **, and the repo carries 6 MB of dead binaries.** `.github/workflows/` holds only an auto-assign action; `.circleci/config.yml` has unsubstituted `{{ORG_NAME}}` placeholders (line 9) and pins Go 1.16; `.golangci.yml` names linters that no longer exist; `dependabot.yml` watches GitHub Actions, not `gomod`. `rh.exe` (5,479,424 B) was **Resource Hacker**, added 2019 for a deleted Travis icon-embedding step and referenced by nothing in the tree — removed at `1866ffa7` (21 Aug); `d2logo.ico` and `d2discord.png` remain orphaned.
...
- **No CI runs** *(as found; resolved at M2.4, 21 Aug 2026 — `.github/workflows/ci.yml` runs gofmt/vet/build/test and the Article V fixture check on ubuntu and windows; `.circleci/`, `.golangci.yml` and the auto-assign workflow were deleted. First run found that 7 of the 20 test packages panic headless on Linux: `d2common/d2util/debug_print.go` imports ebiten, whose init initialises GLFW, which needs a DISPLAY — the Linux test step runs under `xvfb-run`, exactly as the dead CircleCI config did)*. As found: `.github/workflows/auto-author-assign.yml` only; `.circleci/config.yml` was template-broken (`{{ORG_NAME}}`, line 9) and pinned Go 1.16; `.travis.yml` deleted 2021-03-04 (`fbd675c0`).
- **`.golangci.yml`** *(deleted at M2.4)* enabled 39 linters incl. removed ones (`deadcode`, `structcheck`, `varcheck`, `golint`, `interfacer`); current golangci-lint errors on the names before linting. `govet` disabled `sigchanyzer`, which `d7c88c99` fixed anyway. Lint can return as a deliberate decision with a fresh config; nothing in the plan depends on it.
...
4. **M2.4 hooks:** ~~add a GitHub Actions build/vet/test workflow alongside the write-blocking hooks; fix or delete `.circleci/` and `.golangci.yml`~~ — **done, 21 Aug** (`tools/strigoihook`, `.claude/settings.json`, `.github/workflows/ci.yml`; the dead configs deleted).
