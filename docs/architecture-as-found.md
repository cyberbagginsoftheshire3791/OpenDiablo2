...
4. **M2.4 hooks:** ~~add a GitHub Actions build/vet/test workflow alongside the write-blocking hooks; fix or delete `.circleci/` and `.golangci.yml`~~ — **done, 22 Aug** (`tools/strigoihook`, `.claude/settings.json`, `.github/workflows/ci.yml`; the dead configs deleted).
