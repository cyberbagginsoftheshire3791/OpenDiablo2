...
- **No CI runs** *(as found; resolved at M2.4, 22 Aug 2026 — `.github/workflows/ci.yml` runs gofmt/vet/build/test and the Article V fixture check on ubuntu and windows; `.circleci/`, `.golangci.yml` and the auto-assign workflow were deleted)*. As found: `.github/workflows/auto-author-assign.yml` only; `.circleci/config.yml` was template-broken (`{{ORG_NAME}}`, line 9) and pinned Go 1.16; `.travis.yml` deleted 2021-03-04 (`fbd675c0`).
