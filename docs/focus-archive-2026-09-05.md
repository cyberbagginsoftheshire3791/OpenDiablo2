# FOCUS archive — frozen 5 September 2026

This is the body of `.claude/FOCUS.md` as it stood at commit `4e351a8f`,
before that file was cut back to the one screen it always claimed to be.
It had reached 528 lines and was printed into **every** Claude Code session
by the SessionStart hook, which made it the most expensive stale document
in the project: it announced of itself, in its own line 32, that it was one
milestone behind.

**This file is FROZEN. Nothing is ever appended to it.** New status goes to
`next-session-prompt.md`; new history goes to `state-history.md`; durable
facts go to `state.md`. All three live in
`OneDrive\Desktop\Video Game\Claude doc outputs\` and are mirrored into the
claude.ai project.

**Read it only when the milestone you are working on is one it covers**, and
prefer the build note for that milestone over anything here. Where this file
and a signed build note disagree, the build note wins; where either and the
engine disagree, the engine wins.

## What is in here

| Lines (of the original) | Subject |
|---|---|
| 32-51 | The five heads that landed 28 Aug - 3 Sep, and the M4.5 step framing |
| 52-172 | M4.5 step 3 (the NPC body) and M4.2 (survival meters) |
| 173-232 | M4.3a / M4.3b closure, the build order, M4.7's scope, the two approved side bursts |
| 233-341 | M4.3b measurements (notice, light, rout) and the M4.3a unit-scale bug postmortem |
| 342-464 | M4.3a in full: the overworld-already-exists finding, raycast to A*, three latent engine bugs, M4.3b's seams, the research gate |
| 465-486 | The map-fire scoping note: ask 2's ruling, D2's object table, what still blocks lighting code |
| 487-517 | The black floor, `addDT1`'s missing return, `DecodeTileGfxData` — the parked DT1-milestone bugs |
| 518-528 | The parked list and the "do not" list, both carried forward into the new FOCUS.md |

**Two things in here are known stale**, found by a triage pass on 5 Sep:
its own `Updated:` line said 2026-09-01 when its last touching commit was
2026-09-03; and the map-fire counting pass is described as approved-and-pending
in one paragraph when an earlier paragraph already reports the result of the
commit that did it (`dd2b7d99`, ask 1: about twenty-one lit objects placed,
none lit in the mode it is placed in).
